...
var prefix = ayarlar.prefix;
  ...
  if (!user) return message.channel.send(`Sunucudan yasaklamak istediğiniz kullanıcıyı etiketlemelisiniz; \`${prefix}ban @Kullanıcı\` `);
  if (user.id === message.author.id) return message.channel.send('Kendini Nasıl Yasaklamayı Düşünüyorsun Gerizekalı');
 ...
 if (!message.guild.member(user).bannable) return message.channel.send(`Bu kişiyi sunucudan yasaklayamıyorum çünkü \`benden daha yüksek bir role sahip\` ya da \`bana gerekli yetkileri vermedin\`.`);
   ...
   if (!message.guild.member(user).bannable) return message.channel.send('Benden Sunucudaki Yetkilileri Yasaklamamı Bekleme Dostum!');
