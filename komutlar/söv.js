exports.run = async (client, msg, args) => {
 ...
 description: ('<a:retg:839073832840986645> Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle (Beni Etiketlersen Azına Sıçarım).')
}});
  if(member.id === "782246367204605953")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0x0000ff + 1)),
 description: ('<a:retg:839073832840986645> Amına Koduğumun Orospusu Seni Sen Kimsinde Benim Kralıma Söversin!')
}})
  if(member.id === client.user.id){
    msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`<a:retg:839073832840986645> Siktir Duygusuz Pezevenk Bide Bana Sövecek!!`)
}})
  }
  else{
  msg.channel.send({embed: {
